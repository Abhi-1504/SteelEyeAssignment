...
class TestSteelEye(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Path to xml files
        self.xmlfilepath = os.path.join(os.getcwd(), 
                                        TestSteelEye.download_path)
        
        # Path to csv file
        self.csvfile = os.path.join(os.getcwd(), 
                                        TestSteelEye.csv_path)
        
    def test_download(self):
        """Function to test download function"""
        
        # Test for all correct data
        self.assertEqual(download(TestSteelEye.url, 
                                 self.xmlfilepath,
                                 'sourcefile.xml'),
                        self.xmlfilepath + os.sep +'sourcefile.xml')
        
        # Test for incorrect url
        self.assertEqual(download('http://example.com', 
                                 self.xmlfilepath,
                                 'sourcefile.xml'), '')
    ...
    def test_unzip_file(self):
        ...
        zipped_file = os.path.join(self.xmlfilepath, 
                                  'DLTINS_20210117_01of01.zip')
        # Test for correct data
    ...
    def test_create_csv(self):
        ...
        xml_file = os.path.join(self.xmlfilepath, 
    ...
    def aws_s3_upload(self):
        ...
        # Test for correct data
        self.assertTrue(aws_s3_upload(csv_file, self.region_name, 
        ...
        self.assertFalse(aws_s3_upload(csv_file, 'useast', 
                                      self.aws_access_key_id, 
                                      self.aws_secret_access_key,
                                      self.bucket_name)
                         )
        ...
        self.assertFalse(aws_s3_upload(csv_file, self.region_name, 
                                      self.aws_access_key_id, 
                                      self.aws_secret_access_key,
